...
extern "C" {
	...
	#include "locationsystem/RaspiCamControl.h"
	#include "locationsystem/RaspiPreview.h"
	#include "locationsystem/RaspiCLI.h"
}
	...
	)
{
	...
	state->timeout		= 1000;		// capture time : here 1s
	state->width		= 1280;		// use a multiple of 320
	state->height		= 720;		// use a multiple of 240
	...
	)
{
	...
	if( pData )
	{
		if( buffer->length )
		{
			...
			int w=pData->pstate->width, h=pData->pstate->height;
			memcpy( cvImage_ptr->image.data, buffer->data, sizeof(unsigned char)*w*h );
			cvImage_ptr->encoding = sensor_msgs::image_encodings::MONO8;
...
*
*/
static void check_disable_port(
	MMAL_PORT_T *port
	)
{
	...
	)
{
	...
	cvImage_ptr->image.create( h, w, CV_8UC1 );
	...
	else
	{
		...
		/* ROS System */
		ROS_INFO( "Pi Camera Capture Start" );
		ROS_INFO( "SURF" );
		ros::NodeHandle nh_;
		...
		{
			ROS_INFO( "loop" );
			cvImage_ptr->toImageMsg( imgOrg );
			surf.ipSurf.clear();
			surf.Surf( imgOrg, surf.ipSurf );

			// file.WriteImage( imgOrg, "/home/pi/test.bmp" );
			// imgOrg.data.clear();
			// return 0;
			
			//sensor_msgs::Image imgRev = imgOrg;
			//improc.Reverse( imgRev );


			// image_pub_.publish( imgRev );
			// ros::spinOnce();
		}

		// Now wait until we need to stop
		vcos_sleep( state.timeout );
		ROS_INFO( "Pi Camera Capture End" );
