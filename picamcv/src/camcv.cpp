...
#include "picamcv/RaspiCamControl.h"
#include "picamcv/RaspiPreview.h"
#include "picamcv/RaspiCLI.h"
}
	...
	)
{
	...
	if( pData )
	{
		if( buffer->length )
		{
			...
			cvImage_ptr->image = cvarrToMat( py );
			// ROS_INFO( cvImage_ptr->encoding.c_str() );
			image_pub_.publish( cvImage_ptr->toImageMsg() );
			// sensor_msgs::ImagePtr msg = cvImage_ptr->toImageMsg();
			// image_pub_.publish( msg );

			// imshow( "Capture Window", cvImage_ptr->image );
			// waitKey( 1 );
